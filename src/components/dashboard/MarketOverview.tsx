...
export const MarketOverview = () => {
  ...
  const renderMiniChart = (data: number[]) => {
    ...
    return <svg width={width} height={height} className="ml-auto">
  ...
  return <Card className="p-6 bg-gradient-surface border-border shadow-glow-soft hover:shadow-glow transition-all duration-300 hover-lift cursor-pointer" onClick={() => navigate('/trade')}>
      ...
      <div className="space-y-4">
        {marketData.map(market => <div key={market.symbol} className="group">
            ...
            {market.symbol !== marketData[marketData.length - 1].symbol && <div className="h-px bg-border/50 mt-3" />}
          </div>)}
      </div>
